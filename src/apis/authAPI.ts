...
// 로그아웃 API
export const patchSignOutAPI = async () => {
  try {
    const res = await client.patch('/auth/members/sign-out');
    return res;
  } catch (error: any) {
    if (error.response) {
      // 서버 응답이 있는 경우 (오류 상태 코드 처리)
...
// 회원탈퇴 API
export const deleteMembersAPI = async () => {
  try {
    const res = await client.delete('/auth/members');
    return res;
  } catch (error: any) {
    if (error.response) {
      // 서버 응답이 있는 경우 (오류 상태 코드 처리)
